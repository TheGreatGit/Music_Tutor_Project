...
export const makeBooking = async (req, res, next) => {
  try {
    ...
    const IdArray = [
      { key: "tutor_id", value: tutor_id },
      { key: "student_id", value: student_id },
      { key: "instrument_id", value: instrument_id },
      { key: "teaching_format_id", value: teaching_format_id },
      { key: "teaching_type_id", value: teaching_type_id },
      { key: "skill_level_id", value: skill_level_id },
    ];
    ...
    for (const { key, value } of IdArray) {
      ...
      if (!Number.isInteger(parsed) || parsed <= 0) {
        ...
        return next(
          new Error(
            `Invalid field: ${key}. ${key} must be a number grater than 0`,
          ),
        );
      }
    ...
    if (
      Number.isNaN(startTimeAsDate.getTime()) ||
      Number.isNaN(endTimeAsDate.getTime())
    ) {
      res.status(400);
    ...
    const durationInMinutes =
      (endTimeAsDate.getTime() - startTimeAsDate.getTime()) / (1000 * 60);
    if (
      durationInMinutes < MIN_LESSON_DURATION_IN_MINUTES ||
      durationInMinutes > MAX_LESSON_DURATION_IN_MINUTES
    ) {
      res.status(400);
      return next(
        new Error(
          `Appointments must be between ${MIN_LESSON_DURATION_IN_MINUTES} and ${MAX_LESSON_DURATION_IN_MINUTES} minutes`,
        ),
      );
    }

    // check for appointment conflicts before trying to insert the appointment
    const { rows: conflict } = await query(checkConflictsQuery, [
      parsedIds.tutor_id,
      parsedIds.student_id,
      booking_start_time,
      booking_end_time,
    ]);
    ...
    if (conflict.length > 0) {
      ...
      return next(
        new Error(
          `${who} already has a confirmed booking that overlaps this slot`,
        ),
      );
    }
    // use the values from parsedIds
    const { rows } = await query(makeBookingQuery, [
      parsedIds.tutor_id,
      parsedIds.student_id,
      parsedIds.instrument_id,
      booking_start_time,
      booking_end_time,
      booking_status,
...
export const cancelBookingById = async (req, res, next) => {
  try {
    ...
    if (req.user?.student_id) {
      participantId = Number(req.user.student_id);
      participantRole = "student";
    }
    if (req.user?.tutor_id) {
      participantId = Number(req.user.tutor_id);
    ...
    const cancelResult = await query(cancelBookingByIdQuery, [
      bookingId,
      participantId,
      participantRole,
    ]);
    ...
    const isParticipant =
      ...
      (participantRole === "student" && participantId === booking.student_id);
    if (!isParticipant) {
      res.status(403);
    ...
    if (booking.started_or_passed) {
      res.status(409);
      return next(
        new Error("LEsson has already passed or begun and cannot be cancelled"),
      );
    }
    ...
    if (booking.within_24h) {
      res.status(409);
      return next(
        new Error("Bookings cannot be cancelled within 24 hours of start time"),
      );
    }

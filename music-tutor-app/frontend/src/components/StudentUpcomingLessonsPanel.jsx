...
const StudentUpcomingLessonsPanel = ({ user }) => {
  ...
  const [studentBookingsError, setStudentBookingsError] = useState(null);
...
*/
  ...
  useEffect(() => {
    ...
    const getStudentBookings = async () => {
      ...
      setStudentBookingsError(null);

      try {
        const res = await fetch(
          `http://localhost:3000/api/bookings/students/${user.student_id}`,
          { credentials: "include", signal: controller.signal },
        );
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data?.message || "Failed to fetch student bookings");
        }

        setStudentBookings(bookingShaper(data));
      } catch (error) {
        if (error.name === "AbortError") {
          console.log("Student bookings fetch cancelled");
        } else {
          console.log("studentBookings fetch erroor", error);
          setStudentBookingsError(
            error?.message || "Failed to fetch student bookings",
          );
          setStudentBookings([]);
        }
      } finally {
...
*/
  ...
  */
  ...
  if (studentBookingsLoading) {
    return <p>Loading student bookings</p>;
  }
  if (studentBookingsError) {
    return <p className="text-red-600">{studentBookingsError}</p>;
  }
  ...
  return (
    <>
      ...
      <button
        ...
        className="px-3 py-1 border rounded-lg  font-medium cursor-pointer bg-indigo-600 text-white hover:bg-indigo-700"
      >
        Clear selection
      </button>
              ...
              {selectedEvent && !selectedEvent?.isDraft && (
                <>
                  <button
                    ...
                    className={`mt-3 px-3 py-1 border rounded-md bg-red-700 text-white font-medium
                ${
...
export default StudentUpcomingLessonsPanel;
